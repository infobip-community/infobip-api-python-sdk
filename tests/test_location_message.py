...
LOCATION_MESSAGE_ENDPOINT = "/whatsapp/1/message/location"
...
def test_send_location_message_with_provided_client__returns_raw_response(
    httpserver, http_test_client, ok_content, response_ok
):
    httpserver.expect_request(
        LOCATION_MESSAGE_ENDPOINT, method="POST"
    ).respond_with_response(response_ok)

    whatsapp_client = WhatsAppChannel.from_provided_client(
        http_test_client(httpserver.url_for("/"))
    )
    response = whatsapp_client.send_location_message(LocationMessageBodyFactory.build())
...
def test_send_location_message_with_auth_params__returns_whatsapp_response(
    httpserver,
    http_test_client,
    raw_response,
    response_object,
